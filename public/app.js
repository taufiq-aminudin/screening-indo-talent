...
async function api(path,opt={}){const h=new Headers(opt.headers||{});const token=sessionStorage.getItem('ats_session_token');if(token){if(!h.has('Authorization'))h.set('Authorization','Bearer '+token);if(!h.has('X-Session-Token'))h.set('X-Session-Token',token);}const r=await fetch(path,{credentials:'include',...opt,headers:h});const data=await r.json().catch(()=>({}));if(!r.ok){const base=data.error||'request_failed';const message=data.detail?base+': '+data.detail:base;throw new Error(message)}return data}
...
async function loadBilling(){
  try{const d=await api('/api/billing');const w=d.wallet||{};window.billingState=d;$('#creditBalance').textContent=Number(w.balance||0).toLocaleString('id-ID')+' credits';
    const pi=d.payment_instructions||{};
    $('#paymentInstructions').innerHTML='<b>Payment instructions</b><div>Bank: '+esc(pi.bank_name||'Bank Mandiri')+'</div><div>Account name: '+esc(pi.account_name||'PT Surya Utama International')+'</div><div class="payment-account-number">Account number: <strong>'+esc(pi.account_number||'-')+'</strong></div>'+(pi.qris_url?'<div style="margin-top:8px"><a href="'+esc(pi.qris_url)+'" target="_blank" rel="noopener">Open QRIS</a></div>':'')+'<small>Transfer the exact package amount, then upload your transfer proof when submitting payment.</small>';
    $('#creditPackages').innerHTML=(d.packages||[]).map((p,i)=>'<div class="credit-package '+(i===1?'featured':'')+'"><div class="pool-kicker">'+(p.tag||'')+'</div><h3>'+esc(p.name)+'</h3><div class="credit-price">Rp '+Number(p.price_idr).toLocaleString('id-ID')+'</div><b>'+Number(p.credits).toLocaleString('id-ID')+' credits</b><div class="credit-tag">AI Screening Credits. Provider tokens remain internal.</div><div class="package-payment-note"><b>Pay by Bank Mandiri</b><span>PT Surya Utama International</span><strong>185-00-1084321-4</strong><small>Transfer exact amount, then upload proof.</small></div><button class="btn credit-buy" onclick="buyCredits(\''+esc(p.code)+'\')">Buy & submit payment</button></div>').join('');
    ...
    $('#creditOrders').innerHTML=(d.orders||[]).length?(d.orders||[]).map(x=>'<tr><td>'+esc(x.package_code)+'</td><td>'+Number(x.credits).toLocaleString('id-ID')+'</td><td>Rp '+Number(x.amount_idr).toLocaleString('id-ID')+'</td><td><span class="pill '+esc(x.status)+'">'+esc(x.status.replaceAll('_',' '))+'</span></td><td>'+esc(x.payment_reference||'-')+'</td><td>'+esc(new Date(x.created_at).toLocaleString('id-ID'))+'</td><td><div class="job-actions">'+(x.status==='awaiting_payment'?'<button class="btn secondary" type="button" onclick="openPayment(\''+esc(x.id)+'\')">Pay / Submit</button>':x.status==='payment_submitted'?'<span class="small">Payment submitted</span>':'')+(x.payment_proof_key?'<a class="btn secondary" target="_blank" rel="noopener" href="/api/billing/orders/'+encodeURIComponent(x.id)+'/payment-proof">View proof</a>':'')+'</div></td></tr>').join(''):'<tr><td colspan="7" class="job-empty">No orders yet.</td></tr>';
    const table=document.querySelector('#creditOrders')?.closest('table'); if(table){const thead=table.querySelector('thead tr'); if(thead&&!thead.querySelector('.payment-ref-head'))thead.innerHTML='<th>Package</th><th>Credits</th><th>Amount</th><th>Status</th><th class="payment-ref-head">Payment reference</th><th>Created</th><th>Action</th>';}
...
window.buyCredits=async code=>{try{const r=await api('/api/billing/orders',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({package_code:code})});await loadBilling();openPayment(r.order_id)}catch(e){alert('Order gagal: '+e.message)}};
window.openPayment=function(orderId){const d=window.billingState||{};const o=(d.orders||[]).find(x=>x.id===orderId);if(!o)return;const pi=d.payment_instructions||{};const existing=document.getElementById('paymentModal');if(existing)existing.remove();const el=document.createElement('div');el.id='paymentModal';el.className='modal';el.innerHTML='<div class="modal-backdrop" onclick="closePayment()"></div><div class="modal-card"><div class="modal-head"><div><div class="pool-kicker">PAYMENT</div><h2>Complete payment</h2><p class="muted">Order '+esc(o.id)+' · '+esc(o.package_code)+' · Rp '+Number(o.amount_idr).toLocaleString('id-ID')+'</p></div><button class="btn secondary" type="button" onclick="closePayment()">Close</button></div><div class="payment-box"><b>Payment instructions</b><div>Bank: '+esc(pi.bank_name||'Bank Mandiri')+'</div><div>Account name: '+esc(pi.account_name||'PT Surya Utama International')+'</div><div class="payment-account-number">Account number: <strong>'+esc(pi.account_number||'-')+'</strong></div>'+(pi.qris_url?'<div style="margin-top:8px"><a href="'+esc(pi.qris_url)+'" target="_blank" rel="noopener">Open QRIS</a></div>':'')+'</div><form onsubmit="submitPayment(event,\''+esc(o.id)+'\')"><label>Payment method</label><select id="payMethod" class="input"><option value="bank_transfer">Bank Transfer</option><option value="qris">QRIS</option></select><label>Transfer / payment reference <span class="small">(optional)</span></label><input id="payRef" class="input" placeholder="e.g. transfer reference / receipt number"><label>Upload bukti transfer <strong class="required-mark">*</strong></label><div class="proof-upload"><input id="payProof" class="input" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required><small class="small">Wajib. JPG, PNG, WEBP atau PDF · maksimum 5 MB. Bukti ini akan dikirim ke Super Admin untuk verifikasi.</small><div id="paymentFileName" class="small" style="margin-top:6px">Belum ada file</div></div><label>Note (optional)</label><textarea id="payNote" class="input" rows="3" placeholder="Optional payment note"></textarea><div class="form-actions"><button class="btn" type="submit">Submit payment</button></div><p id="paymentSubmitMsg" class="muted"></p></form></div>';document.body.appendChild(el);const pf=document.getElementById('payProof');pf?.addEventListener('change',()=>{const m=document.getElementById('paymentFileName');if(m)m.textContent=pf.files?.[0]?('Selected: '+pf.files[0].name):'Belum ada file'});};
window.closePayment=function(){document.getElementById('paymentModal')?.remove()};
window.submitPayment=async function(e,id){e.preventDefault();const msg=document.getElementById('paymentSubmitMsg');if(msg)msg.textContent='Uploading payment proof...';try{const proof=document.getElementById('payProof').files?.[0];if(!proof)throw Error('Upload bukti transfer wajib dilakukan.');const fd=new FormData();fd.append('payment_method',document.getElementById('payMethod').value);fd.append('payment_reference',document.getElementById('payRef').value);fd.append('payment_note',document.getElementById('payNote').value);fd.append('payment_proof',proof);await api('/api/billing/orders/'+encodeURIComponent(id)+'/submit-payment',{method:'POST',body:fd});closePayment();alert('Payment submitted. Bukti transfer sudah diterima dan akan diverifikasi Super Admin.');await loadBilling()}catch(x){if(msg)msg.textContent='Submit payment gagal: '+x.message;else alert('Submit payment gagal: '+x.message)}};
