...
window.openPayment=function(orderId){const d=window.billingState||{};const o=(d.orders||[]).find(x=>x.id===orderId);if(!o)return;const pi=d.payment_instructions||{};const existing=document.getElementById('paymentModal');if(existing)existing.remove();const el=document.createElement('div');el.id='paymentModal';el.className='modal';el.innerHTML='<div class="modal-backdrop" onclick="closePayment()"></div><div class="modal-card"><div class="modal-head"><div><div class="pool-kicker">PAYMENT</div><h2>Complete payment</h2><p class="muted">Order '+esc(o.id)+' · '+esc(o.package_code)+' · Rp '+Number(o.amount_idr).toLocaleString('id-ID')+'</p></div><button class="btn secondary" type="button" onclick="closePayment()">Close</button></div><div class="payment-box"><b>Payment instructions</b><div>Bank: '+esc(pi.bank_name||'Bank Transfer')+'</div><div>Account name: '+esc(pi.account_name||'-')+'</div><div>Account number: '+esc(pi.account_number||'-')+'</div>'+(pi.qris_url?'<div style="margin-top:8px"><a href="'+esc(pi.qris_url)+'" target="_blank" rel="noopener">Open QRIS</a></div>':'')+'</div><form onsubmit="submitPayment(event,\''+esc(o.id)+'\')"><label>Payment method</label><select id="payMethod" class="input"><option value="bank_transfer">Bank Transfer</option><option value="qris">QRIS</option></select><label>Transfer / payment reference</label><input id="payRef" class="input" required placeholder="e.g. transfer reference / receipt number"><label>Note (optional)</label><textarea id="payNote" class="input" rows="3" placeholder="Optional payment note"></textarea><div class="form-actions"><button class="btn" type="submit">Submit payment</button></div></form></div>';document.body.appendChild(el)};
window.closePayment=function(){document.getElementById('paymentModal')?.remove()};
window.submitPayment=async function(e,id){e.preventDefault();try{await api('/api/billing/orders/'+encodeURIComponent(id)+'/submit-payment',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payment_method:document.getElementById('payMethod').value,payment_reference:document.getElementById('payRef').value,payment_note:document.getElementById('payNote').value})});closePayment();alert('Payment submitted. Super Admin akan memverifikasi pembayaran sebelum credits diaktifkan.');await loadBilling()}catch(x){alert('Submit payment gagal: '+x.message)}};
