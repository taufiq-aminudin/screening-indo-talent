...
async function api(path,opt={}){const h=new Headers(opt.headers||{});const token=sessionStorage.getItem('ats_session_token');if(token&&!h.has('Authorization'))h.set('Authorization','Bearer '+token);const r=await fetch(path,{credentials:'include',...opt,headers:h});const data=await r.json().catch(()=>({}));if(!r.ok){const base=data.error||'request_failed';const message=data.detail?base+': '+data.detail:base;throw new Error(message)}return data}
