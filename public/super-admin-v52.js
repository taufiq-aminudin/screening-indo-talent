...
async function api(p,o={}){const h=new Headers(o.headers||{});const token=sessionStorage.getItem('ats_admin_token');if(token&&!h.has('Authorization'))h.set('Authorization','Bearer '+token);const r=await fetch(p,{credentials:'include',cache:'no-store',...o,headers:h});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||d.detail||'request_failed');return d}
