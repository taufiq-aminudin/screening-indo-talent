...
if(form){
  ...
  form.onsubmit=async e=>{e.preventDefault();const m=$('#m');if(m)m.textContent='Signing in...';try{const login=await api('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:$('#e').value,password:$('#p').value})});if(login.admin_token)sessionStorage.setItem('ai_screening_admin_token',login.admin_token);await load()}catch(x){if(m)m.textContent=x.message==='admin_not_configured'?'LOGIN BLOCKED: Secret SUPER_ADMIN_EMAIL dan SUPER_ADMIN_PASSWORD/HASH belum tersedia pada Worker deployment ini.':x.message==='invalid_admin_credentials'?'Email atau password Super Admin tidak cocok.':x.message==='admin_session_failed'?'Login berhasil diverifikasi tetapi session cookie gagal dibuat: '+x.message:x.message}};
}
load();
