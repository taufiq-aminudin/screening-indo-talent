...
async function api(p,o={}){const h=new Headers(o.headers||{});const token=sessionStorage.getItem('ats_admin_token');if(token&&!h.has('Authorization'))h.set('Authorization','Bearer '+token);const r=await fetch(p,{credentials:'same-origin',cache:'no-store',...o,headers:h});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||d.detail||'request_failed');return d}
...
async function logout(){await api('/api/auth/logout',{method:'POST'});sessionStorage.removeItem('ats_admin_token');sessionStorage.removeItem('ats_session_token');location.reload()}
const form=$('#f');
if(form){
  ...
  form.onsubmit=async e=>{e.preventDefault();const m=$('#m');if(m)m.textContent='Signing in...';try{const r=await api('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:$('#e').value,password:$('#p').value})});if(r.admin_token)sessionStorage.setItem('ats_admin_token',r.admin_token);await load()}catch(x){if(m)m.textContent=x.message==='admin_not_configured'?'Bootstrap Super Admin belum terlihat oleh deployment Worker ini. Pastikan Secret SUPER_ADMIN_EMAIL dan SUPER_ADMIN_PASSWORD sudah tersimpan pada Worker yang sama lalu deploy versi terbaru.':x.message==='invalid_admin_credentials'?'Email atau password Super Admin tidak cocok. Gunakan credential Super Admin, bukan login perusahaan.':x.message}};
}
load();
