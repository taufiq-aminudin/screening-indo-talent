...
function orderRows(orders){
  ...
  return orders.map(x=>'<tr><td><b>'+esc(x.company_name||'-')+'</b><div class="small">'+esc(x.company_email||'')+'</div></td><td>'+esc(x.package_code)+'</td><td>'+num(x.credits)+'</td><td>'+money(x.amount_idr)+'</td><td>'+statusPill(x.status)+'</td><td>'+esc(x.payment_method||'-')+'</td><td>'+esc(x.payment_reference||'-')+'</td><td>'+esc(x.created_at?new Date(x.created_at).toLocaleString('id-ID'):'-')+'</td><td>'+(['pending','awaiting_payment','payment_submitted'].includes(x.status)?'<div class="actions"><button class="btn" type="button" onclick="approveOrder(&quot;'+esc(x.id)+'&quot;)">Approve</button><button class="btn danger" type="button" onclick="rejectOrder(&quot;'+esc(x.id)+'&quot;)">Reject</button></div>':'<span class="small">'+(x.paid_at?esc(new Date(x.paid_at).toLocaleString('id-ID')):'Processed')+'</span>')+'</td></tr>').join('');
}
