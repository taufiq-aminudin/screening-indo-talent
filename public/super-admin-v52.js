...
const form=$('#f');
if(form){
  api('/api/admin/config-status').then(x=>{const cfg=$('#cfg');if(!cfg)return;if(x.ok){cfg.textContent='Secure configuration ready · '+x.auth_mode+' · '+x.configured_email_masked;cfg.style.color='#087443'}else{cfg.textContent='Configuration incomplete · SUPER_ADMIN_EMAIL / SUPER_ADMIN_PASSWORD belum tersedia pada deployment ini';cfg.style.color='#b42318'}}).catch(e=>{const cfg=$('#cfg');if(cfg)cfg.textContent='Configuration status unavailable: '+(e.message||'request failed')});
  form.onsubmit=async e=>{e.preventDefault();const m=$('#m');if(m)m.textContent='Signing in...';try{await api('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:$('#e').value,password:$('#p').value})});await load()}catch(x){if(m)m.textContent=x.message==='admin_not_configured'?'LOGIN BLOCKED: Secret SUPER_ADMIN_EMAIL dan SUPER_ADMIN_PASSWORD/HASH belum tersedia pada Worker deployment ini.':x.message==='invalid_admin_credentials'?'Email atau password Super Admin tidak cocok.':x.message==='admin_session_failed'?'Login berhasil diverifikasi tetapi session cookie gagal dibuat: '+x.message:x.message}};
}
load();
