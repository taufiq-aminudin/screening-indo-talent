...
async function api(p,o={}){const h=new Headers(o.headers||{});const token=sessionStorage.getItem('ats_admin_token');if(token){if(!h.has('Authorization'))h.set('Authorization','Bearer '+token);if(!h.has('X-Admin-Token'))h.set('X-Admin-Token',token);}const r=await fetch(p,{credentials:'include',cache:'no-store',...o,headers:h});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||d.detail||'request_failed');return d}
...
if(form){
  ...
  // Initial Super Admin login uses the native form POST so the HttpOnly cookie set by the Worker is stored before the dashboard loads.
}
load();
