...
function orderRows(orders){
  if(!(orders||[]).length)return '<tr><td colspan="9" class="empty">No credit purchase requests found.</td></tr>';
  return orders.map(x=>'<tr><td><b>'+esc(x.company_name||'-')+'</b><div class="small">'+esc(x.company_email||'')+'</div></td><td>'+esc(x.package_code)+'</td><td>'+num(x.credits)+'</td><td>'+money(x.amount_idr)+'</td><td>'+statusPill(x.status)+'</td><td>'+esc(x.payment_method||'-')+'</td><td>'+esc(x.payment_reference||'-')+'</td><td>'+esc(x.created_at?new Date(x.created_at).toLocaleString('id-ID'):'-')+'</td><td><div class="actions">'+(x.payment_proof_key?'<a class="btn secondary" target="_blank" rel="noopener" href="/api/billing/orders/'+encodeURIComponent(x.id)+'/payment-proof">View proof</a>':'<span class="small">No proof</span>')+(['pending','awaiting_payment','payment_submitted'].includes(x.status)?'<button class="btn" type="button" onclick="approveOrder(&quot;'+esc(x.id)+'&quot;)">Approve</button><button class="btn danger" type="button" onclick="rejectOrder(&quot;'+esc(x.id)+'&quot;)">Reject</button>':'')+'</div></td></tr>').join('');
}
...
if(form){
  ...
  form.onsubmit=async e=>{e.preventDefault();const m=$('#m');if(m)m.textContent='Signing in...';try{await api('/api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:$('#e').value,password:$('#p').value})});await load()}catch(x){if(m)m.textContent=x.message==='admin_not_configured'?'Bootstrap Super Admin belum terlihat oleh deployment Worker ini. Pastikan Secret SUPER_ADMIN_EMAIL dan SUPER_ADMIN_PASSWORD sudah tersimpan pada Worker yang sama lalu deploy versi terbaru.':x.message==='invalid_admin_credentials'?'Email atau password Super Admin tidak cocok. Gunakan credential Super Admin, bukan login perusahaan.':x.message}};
}
load();
