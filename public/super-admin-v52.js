...
async function api(p,o={}){const h=new Headers(o.headers||{});const r=await fetch(p,{credentials:'include',cache:'no-store',...o,headers:h});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||d.detail||'request_failed');return d}
