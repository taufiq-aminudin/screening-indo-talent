...
async function ensureCommercialSchema(db:D1Database){
  await db.batch([
    ...
    db.prepare("CREATE TABLE IF NOT EXISTS credit_orders (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, package_code TEXT NOT NULL, credits INTEGER NOT NULL, amount_idr INTEGER NOT NULL, status TEXT NOT NULL DEFAULT 'pending', payment_reference TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, paid_at TEXT)"),
  ...
  try{
    ...
    if(!emailConfigured || !credentialConfigured){
      return c.json({
        ...
        config:{email_configured:emailConfigured,password_configured:Boolean(configuredPassword),password_hash_configured:Boolean(configuredHash),build:"V6.44"}
      },503);
    }
    ...
    try{
      ...
      return c.json({error:"admin_session_failed",detail:String(e?.message||e),stage:stage.step,build:"V6.44"},503);
    }

    // Audit is deliberately best-effort and can never block authentication.
    stage.step="audit";
    await audit(c,u,"admin.login",u.id);
    return c.json({user:u,auth_source:"bootstrap_secret",build:"V6.44"});
  }catch(e:any){
    return c.json({error:"admin_login_failed",detail:String(e?.message||e),stage:stage.step,build:"V6.44"},500);
  }
});
...
function render(o,c,orders){
 ...
 '<div class="card"><h2>AI Screening Credits</h2><div class="packages">'+[{n:'Starter',c:1000,p:99000},{n:'Growth',c:5000,p:399000},{n:'Professional',c:15000,p:999000},{n:'Enterprise',c:50000,p:2999000}].map(x=>'<div class="package"><h3>'+x.n+'</h3><div class="price">'+money(x.p)+'</div><b>'+num(x.c)+' credits</b><div class="tag">Customer-facing credits, not provider tokens.</div></div>').join('')+'</div></div>'+
 '<div class="card"><div class="section-head"><div><h2 style="margin:0 0 4px">Credit Purchase Requests</h2><div class="muted">Process company package requests and activate credits after payment verification.</div></div><div class="filters"><button class="btn secondary" onclick="loadOrders()">Refresh</button><button class="btn" onclick="filterOrders('pending')">Pending ('+pending+')</button><button class="btn secondary" onclick="filterOrders('paid')">Paid ('+paid+')</button><button class="btn secondary" onclick="filterOrders('rejected')">Rejected ('+rejected+')</button><button class="btn secondary" onclick="filterOrders('all')">All</button></div></div><div class="table-wrap"><table class="table"><thead><tr><th>Company</th><th>Package</th><th>Credits</th><th>Amount</th><th>Status</th><th>Payment reference</th><th>Created</th><th>Action</th></tr></thead><tbody id="orderRows">'+orderRows(orders)+'</tbody></table></div></div>'+
 '<div class="card"><h2>Client companies</h2><div class="table-wrap"><table class="table" style="min-width:700px"><thead><tr><th>Company</th><th>Contact</th><th>Credits</th><th>Purchased</th></tr></thead><tbody>'+c.map(x=>'<tr><td><b>'+esc(x.company_name||'-')+'</b></td><td>'+esc(x.email||'-')+'</td><td><span class="pill">'+num(x.balance)+'</span></td><td>'+num(x.lifetime_purchased)+'</td></tr>').join('')+'</tbody></table></div></div>';
}
function orderRows(orders){if(!(orders||[]).length)return '<tr><td colspan="8" class="empty">No credit purchase requests found.</td></tr>';return orders.map(x=>'<tr><td><b>'+esc(x.company_name||'-')+'</b><div class="small">'+esc(x.company_email||'')+'</div></td><td>'+esc(x.package_code)+'</td><td>'+num(x.credits)+'</td><td>'+money(x.amount_idr)+'</td><td>'+statusPill(x.status)+'</td><td>'+esc(x.payment_reference||'-')+'</td><td>'+esc(new Date(x.created_at).toLocaleString('id-ID'))+'</td><td>'+(x.status==='pending'?'<div class="actions"><button class="btn" onclick="approveOrder(''+esc(x.id)+'')">Approve</button><button class="btn danger" onclick="rejectOrder(''+esc(x.id)+'')">Reject</button></div>':'<span class="small">'+(x.paid_at?esc(new Date(x.paid_at).toLocaleString('id-ID')):'Processed')+'</span>')+'</td></tr>').join('')}
async function loadOrders(status='all'){try{const d=await api('/api/admin/orders?status='+encodeURIComponent(status));const rows=$('#orderRows');if(rows)rows.innerHTML=orderRows(d)}catch(e){alert('Gagal memuat orders: '+e.message)}}
