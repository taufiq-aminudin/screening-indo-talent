  ...
  try{
    ...
    if(!emailConfigured || !credentialConfigured){
      return c.json({
        ...
        config:{email_configured:emailConfigured,password_configured:Boolean(configuredPassword),password_hash_configured:Boolean(configuredHash),build:"V6.46"}
      },503);
    }
    ...
    try{
      ...
      return c.json({error:"admin_session_failed",detail:String(e?.message||e),stage:stage.step,build:"V6.46"},503);
    }

    // Audit is deliberately best-effort and can never block authentication.
    stage.step="audit";
    await audit(c,u,"admin.login",u.id);
    return c.json({user:u,auth_source:"bootstrap_secret",build:"V6.46"});
  }catch(e:any){
    return c.json({error:"admin_login_failed",detail:String(e?.message||e),stage:stage.step,build:"V6.46"},500);
  }
});
...
app.get("/super-admin-v47.js", async c=>{
  try{
    if(c.env.ASSETS){
      const r=await c.env.ASSETS.fetch(new Request(new URL("/super-admin-v47.js",c.req.url)));
...
async function superAdminHtml(c:any){
  ...
  if(admin){
    return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Super Admin · ${c.env.APP_NAME}</title><style>body{margin:0;background:#f5f7fb;font-family:Inter,system-ui,sans-serif;color:#10213b}.wrap{max-width:1220px;margin:30px auto;padding:0 18px}.card{background:#fff;border:1px solid #e3e8f0;border-radius:18px;padding:22px;box-shadow:0 8px 28px rgba(16,33,59,.06);margin-bottom:16px}.loading{text-align:center;padding:55px}</style></head><body><div id="root" class="wrap"><div class="card loading"><h2>Super Admin</h2><p>Authenticated. Loading dashboard…</p><noscript><p>JavaScript diperlukan untuk dashboard Super Admin.</p></noscript></div></div><script src="/super-admin-v47.js?v=48" defer></script></body></html>`);
  }
