...
async function ensureCommercialSchema(db:D1Database){
  ...
  for(const sql of [!cs.has("payment_method")?"ALTER TABLE credit_orders ADD COLUMN payment_method TEXT":null,!cs.has("payment_submitted_at")?"ALTER TABLE credit_orders ADD COLUMN payment_submitted_at TEXT":null,!cs.has("payment_note")?"ALTER TABLE credit_orders ADD COLUMN payment_note TEXT":null]) if(sql){try{await db.prepare(sql).run()}catch{}}
}
...
app.post("/api/billing/orders/:id/submit-payment",requireAuth,async c=>{
  try{const u=c.get("user") as AuthUser;await ensureCommercialSchema(c.env.DB);const oid=c.req.param("id");const b=await c.req.json<any>();const method=String(b.payment_method||"bank_transfer").trim().slice(0,40);const ref=String(b.payment_reference||"").trim().slice(0,120);const note=String(b.payment_note||"").trim().slice(0,500);if(!ref)return c.json({error:"payment_reference_required"},400);const o=await c.env.DB.prepare("SELECT id,status FROM credit_orders WHERE id=? AND company_id=? LIMIT 1").bind(oid,u.company_id).first<any>();if(!o)return c.json({error:"order_not_found"},404);if(o.status==='paid')return c.json({error:"order_already_paid"},409);if(!['awaiting_payment','pending'].includes(String(o.status)))return c.json({error:"order_not_payable",status:o.status},409);await c.env.DB.prepare("UPDATE credit_orders SET status='payment_submitted',payment_method=?,payment_reference=?,payment_submitted_at=CURRENT_TIMESTAMP,payment_note=? WHERE id=? AND company_id=?").bind(method,ref,note,oid,u.company_id).run();await audit(c,u,"billing.payment.submit",oid);return c.json({ok:true,order_id:oid,status:"payment_submitted"})}catch(e:any){return c.json({error:"payment_submit_failed",detail:String(e?.message||e)},500)}});
