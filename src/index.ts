...
async function passwordVerify(password:string,stored:string){
  try{
    ...
    if(!Number.isInteger(iterations)||iterations<1||iterations>600000)return false;
    const decode=(v:string)=>{
      const x=String(v||"").replace(/-/g,"+").replace(/_/g,"/");
    ...
    if(b64url(bits)===expected)return true;
    try{
      const standard=(()=>{let b="";for(const v of bits)b+=String.fromCharCode(v);return btoa(b)})();
      if(standard===expected)return true;
    }catch{}
    try{
      const hexExpected=expected.toLowerCase();
      const hexActual=Array.from(bits).map(v=>v.toString(16).padStart(2,"0")).join("");
      if(hexActual===hexExpected)return true;
    }catch{}
    return false;
  }catch{return false}
}
...
</style></head><body><div id="root" class="wrap"><div class="card loading"><h2>Super Admin</h2><p>Authenticated. Loading dashboard…</p><noscript><p>JavaScript diperlukan untuk dashboard Super Admin.</p></noscript></div></div><script src="/super-admin-v52.js?v=61-auth-final" defer></script></body></html>`);
  }
  return c.html(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Super Admin · ${c.env.APP_NAME}</title><style>
...
<script src="/app.js?v=637-auth-final" defer></script></body></html>`));
